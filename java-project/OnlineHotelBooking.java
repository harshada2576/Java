...
InvalidCheckInDateException { 
        try { 
            ...
            if (checkIn.isBefore(today)) { 
                throw new InvalidCheckInDateException("Invalid check-in date! The date must be today or later."); 
            } 
            System.out.println("Room booked successfully for " + checkInDate); 
        } catch (DateTimeParseException e) { 
            throw new InvalidCheckInDateException("Invalid date format! Use YYYY-MM-DD."); 
        } 
    } 
} 
 
// Main Class 
public class OnlineHotelBooking { 
